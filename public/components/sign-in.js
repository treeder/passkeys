...
import { api } from 'api'
import { startRegistration, startAuthentication } from 'https://cdn.jsdelivr.net/npm/@simplewebauthn/browser@10/dist/bundle/index.js'
import { globals, isLoggedIn } from '/js/globals.js'
...
export class SignIn extends LitElement {
    ...
    render() {
        ...
        if (isLoggedIn()) {
            ...
            <div>
                <a href="/">Skip this and create passkey later</a>
            </div>
            </div>`
    ...
    async continue() {
        ...
        try {
            let r = await api(`/auth/email/continue`, {
                method: "POST",
                body: { email: username },
            })
            console.log("r:", r)
    ...
    async signin2(conditionalUI = false) {
        ...
        let challenge
        try {
            challenge = await api(`/auth/passkeys/signin/start`, {
                method: "POST",
                body: {},
            })
            ...
            let cred = await startAuthentication(challenge, conditionalUI)
            ...
            let r = await api(`/auth/passkeys/signin/finish`, {
                method: "POST",
                body: {
                    credential: cred,
                },
    ...
    async createPasskey() {
        ...
        let challenge
        try {
            challenge = await api(`/auth/passkeys/new/start`, {
                method: "POST",
                body: {},
            })
            console.log("challenge:", challenge)
        ...
        try {
            let r = await api(`/auth/passkeys/new/finish`, {
                method: "POST",
                body: {
                    credential: attResp,
                    userID: userID,
